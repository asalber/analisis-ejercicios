...
:::{#exr-particiones-intervalos}
Calcular particiones en $n$ subintervalos de igual tamaño de los siguientes intervalos.
...
## Solución
a.  $P = \left\{ \frac{i}{n}: i=0,1,\ldots,n\right\}$.
a.  $P = \left\{ \pi + \frac{i\pi}{n}: i=0,1,\ldots,n\right\}$.
a.  $P = \left\{ -1 + \frac{2i}{n}: i=0,1,\ldots,n\right\}$.
:::
...
![Integral de Riemann de $f(x)=x^2$ en $[0,1]$.](img/integrales/integral-cuadrado)
:::
...
Como $\underline{\int_0^1} x^3 = \overline{\int_0^1} x^3=\frac{1}{4}$, la función $f(x)=x^3$ es integrable Riemann en $[0,1]$ y $\int_0^1 x^3\, dx = \frac{1}{4}$.
...
![Integral de Riemann de $f(x)=x^3-2x^2$ en $[0,1]$.](img/integrales/integral-2)
:::
...
:::{#exr-espacio-recorrido-movil}
Un vehículo parte del origen y se desplaza en línea recta con velocidad constante de 60 km/h. Calcular el espacio recorrido por el vehículo a las 2 horas. 

Si a partir de las 2 horas el vehículo se mueve con una aceleración constante de 10 km/h$^2$, ¿cuál será la velocidad del vehículo a las 4 horas? ¿Y su posición?
:::

:::{.callout-tip collapse="true"}
## Solución
La velocidad del vehículo es la variación del espacio recorrido respecto al tiempo, de manera que si $s(t)$ es la función que indica el espacio recorrido en cada instante, se tiene que $s'(t)=60$ km/h. Como el vehículo se desplaza de forma continua, $s(t)$ es una función continua en el intervalo $[0,2]$, y aplicando el teorema fundamental del cálculo tenemos que el espacio recorrido es
...
Del mismo modo, la aceleración del vehículo es la variación de la velocidad respeto al tiempo, de manera que si $v(t)$ es la función que indica la velocidad del vehículo en cada instante, se tiene que $v'(t)=10$ m/s$^2$. Aplicando de nuevo el  teorema fundamental del cálculo se tiene que
...
De igual modo podemos obtener la velocidad en cualquier instante del intervalo $[2,4]$.
...
de donde se deduce que $v(t) = v(2) + 10t - 20 = 60 + 10t -20 = 10t+40$. Por tanto, el espacio recorrido por el vehículo en el intervalo $[2,4]$ es
...
y la posición del vehículo a las $4$ horas es $s(4) = s(2) + 140 = 120 + 140 = 260$ km.
:::

:::{#exr-desintegracion-radioactiva}
Un cuerpo humano de peso medio tiene alrededor de $3\cdot 10^{14}$ átomos de carbono 14 en vida. Después de morir, los átomos de C14 comienzan a desintegrarse a un ritmo dado por la función $-3.63\cdot 10^{10}e^{-1.21\cdot 10^{-4}t}$ átomos por año. Si se ha encontrado unos restos humanos con solo $0.3\cdot 10^{14}$ átomos de C14, ¿cuánto tiempo ha pasado desde la muerte de la persona?
:::

:::{.callout-tip collapse="true"}
## Solución
Llamando $c(t)$ a la cantidad de átomos de C14 que contiene el cuerpo en cada instante $t$, se tiene que $c'(t) = -3.63\cdot 10^{10}e^{-1.21\cdot 10^{-4}t}$. Aplicando el teorema fundamental del cálculo, se tiene que la cantidad de átomos de C14 que se desintegran en un intervalo de tiempo $[0,t]$ es
...
$$
donde $g_h$ es la aceleración de la gravedad a una altura $h$ sobre la superficie terrestre, $g_0=9.81$ m/s$^2$ es la aceleración de la gravedad en la superficie terrestre, y $R=6371$ km es el radio de la Tierra.
:::
...
Un depósito con forma de sólido de revolución generado al rotar la gráfica de la función $f(x)=\frac{x^2}{2}$ alrededor del eje $y$ en el intervalo de $0$ a $8$ m, contiene $100000$ l de aceite con una densidad $\delta = 900$ kg/m$^3$. ¿Qué trabajo se realiza al vaciar el depósito por arriba?
:::
...
:::{.callout-tip collapse="true"}
## Solución
Para calcular el [centro de masas de una varilla con densidad variable](https://aprendeconalf.es/analisis-manual/08-integrales.html#centro-de-masas-de-una-varilla-con-densidad-variable) $f(x)$ tenemos que dividir su momento entre su masa, es decir, calcular el cociente de integrales 
...
:::{#exr-funcion-densidad}
La función de densidad de probabilidad de una variable aleatoria $X$ es $f(x)=\frac{k}{(1+x)^4}$ para $x\geq 0$. 

a.  ¿Cuándo debe valer $k$ para que se cumpla que $f$ es una función de densidad?

a.  Calcular la probabilidad de que la variable aleatoria tome un valor en el intervalo $[0,1]$.
...
a.  Para que $f$ sea una función de densidad, la integral de $f$ en todo su dominio debe ser igual a $1$. Por tanto, se tiene
...
a.  La probabilidad de que la variable aleatoria tome un valor en el intervalo $[0,1]$ es

    $$
    P(0\leq X\leq 1) = \int_0^1 \frac{3}{(1+x)^4}\,dx = \left[-\frac{3}{3(1+x)^3}\right]_0^1 = 1 - \frac{1}{2^3} = \frac{7}{8}.
    $$

a.  La mediana es el valor $Me$ que deja acumulada la mitad de la probabilidad, es decir, 
